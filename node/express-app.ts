...
app.post('/getNLP_local', async (req, res) => {

    const req_data = req.body;

    // takes an array of strings, not like getNLP_9
    // req_data.subs
    // req_data.sourceLangCode_G

    // returns:
    // { subtitlesNLP: nice_subs }

    // transform to an array of strings
    let subtitleText: Array<string> = req_data.subs;
    ...
    let tookTime = (new Date()).getTime() - startTime;

    if (tookTime < 10000) {
        console.log(`runUDPIPE took ${tookTime} ms, ${req_data.source} data length: ${JSON.stringify(req_data).length}, lang: ${req_data.sourceLangCode_G}`)
    }
    else {
        console.error(`runUDPIPE took ${tookTime} ms, ${req_data.source} data length: ${JSON.stringify(req_data).length}, lang: ${req_data.sourceLangCode_G}`);
    }
...
app.post('/old_to_new', async (req, res) => {

    const req_data = req.body;

    // console.log('req_data ', JSON.stringify(req_data));

    if(!req_data.tokens) { console.error('!req_data.tokens'); }
    let tokenStrings: string[] =  req_data.tokens;
    /*
    req_data.tokens :
        [
            "No🟃no🟃ADV🟃 ",
    ...
    */
    ...
    if(!req_data.subtitle) { console.error('!req_data.subtitle'); }
    let subtitle: string = req_data.subtitle; // "No me gusta el gorgonzola."

    if(!req_data.langCode_G) { console.error('!req_data.langCode_G'); }
    let langCode_G: string = req_data.langCode_G // "es"

    let newTokens: ud_single[] = [];
    ...
    for(let tokenString of tokenStrings) {
        ...
        if(parts[0].trim() === '') {
            ...
            // This is actually whitespace, regardless of number of parts.
            ...
            */
            ...
            if(parts.length !== 1) {
                ...
                console.error('Not actually whitespace: ', tokenString);
            }

            let newNiceToken: ud_single = { 
                form: parts[0],
                pos: 'WS'
            }

            newTokens.push(newNiceToken);
            continue;
        }
        else if(parts.length !== 3 && parts.length !== 4 && parts.length !== 5) {
            console.error('bad token, parts.length !== 3, 4 or 5', tokenString);
            // But.. continue, what else can we do?
        }
        ...
        if(lemma === '' && pos !== '?') { lemma = parts[0]; }

        if(pos === '') { pos = '_'; }

        if(pos === '?') { pos = '_'; }

        let allowedPOS = ['ADJ','ADP','ADV','AUX','NOUN','PROPN','VERB','DET','SYM','INTJ',
        'CCONJ','PUNCT','X','NUM','PART','PRON','SCONJ','_','WS'];

        if(!allowedPOS.includes(pos)) {
            console.error('bad POS tag: ', tokenString);
        }
        ...
        if(parts.length === 5 && langCode_G == 'ja') {
            // Simple tokens can have 5 parts too.
            // Add kana here
            newNiceToken.translit = parts[4];
        }

        if(parts.length === 4 && langCode_G == 'ja') {
            console.error('Missing kana: ', tokenString);
        }

        newTokens.push(newNiceToken);
    }

    let errorEnum = undefined;

    if(langCode_G.startsWith('zh') || langCode_G.startsWith('th') || langCode_G.startsWith('ko')) {
            
        try {
            const {body}: any = await got.post('http://unix:/root/nlp_server/python/nlp_app.sock:/api/translit/', {
                json: {tokens: newTokens, langCode_G: langCode_G},
                responseType: 'json'
            });
    ...
    newTokens = <ud_single[]> clearLemmas(newTokens);
    newTokens = <ud_single[]> addTokenFrequencies(newTokens, langCode_G);
    newTokens = <ud_single[]> addTokenHashes(newTokens);
...
function addTokenHashes(subTokens: Subtitle_Tokens): Subtitle_Tokens {
    ...
    for (let token of flatList) {
        ...
        if(token.pos !== 'PUNCT' && token.pos !== 'WS') {
            ...
            const text = (token.form || '').toLowerCase();
            token.hash = hashFnv32a(text);
        }
    }
    return subTokens;
}
...
function hashFnv32a(str: string) {
    ...
    return ("0000000" + (hval >>> 0).toString(36)).substr(-8);
}
...
app.post('/count', async (req, res) => {
    ...
    if(req_data.data === undefined) {
        return res.status(401).send({message: `dude. data.`});
    }
    else if(!Array.isArray(req_data.data)) {
        // Yeah we only want arrays now.
        return res.status(204).send();
    }
    else {
        for(let item of req_data.data) {

            dataArray[indexPointer] = item;
            indexPointer += 1;
            if(indexPointer < arraySize) {
                // we good
                // yeah, seems like I'm writing branches like this now.
            }
            else {
                indexPointer = 0;
            }
...
app.get('/count_g', async (req, res) => {

    let versionsObj: any = {};

    for(let i = 0; i < arraySize; i++) {

        let versionString = dataArray[i];

        if(versionString === undefined) { break; }
        
        if(versionsObj[versionString] === undefined) {
            versionsObj[versionString] = 0;
        }

        versionsObj[versionString] += 1;
    }
    ...
    return res.status(200).send(versionsObj);

});

let last_value: any = false;
...
app.post('/last', async (req, res) => {
    ...
    if(req_data.data === undefined) {
        ...
        last_value = req_data.data;
        return res.status(204).send();
    }
});

app.get('/last_g', async (req, res) => {

    return res.status(200).send(last_value);

});

let yk_list: string[] = [];

app.post('/yk', async (req, res) => {
    ...
    if(req_data.data === undefined) {
        ...
        // save file
        let fileName = req_data.data[0];
        let stringData = req_data.data[1];

        if(fileName.length > 1 && stringData.length > 1) {
            fs.writeFileSync(`/yoink/${fileName}`, stringData, 'utf8');

            if(!yk_list.includes(fileName)) {
                yk_list.push(fileName);
            }
        }
...
app.get('/yk_g', async (req, res) => {

    let ret = JSON.stringify(yk_list);
